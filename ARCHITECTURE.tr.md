...
- **Hava boşluklu bölge** (`septum-core`, `septum-mcp`, `septum-api`,
  `septum-web`) — tüm PII işlemleri burada. Bu paketler sıfır dışa
  internet bağımlılığına sahip; `septum-core` özellikle `httpx` /
  `requests` / `urllib` import'unu yasaklar, böylece ham PII kazara
  asla dışa sızmaz.
- **Köprü** (`septum-queue`) — zone'lar arasında yalnızca önceden
  maskelenmiş payload'ları taşır; dosya backend (hava boşluğu varsayılanı)
  veya Redis Streams (`[redis]` ek bileşeni). Ham PII sözleşme gereği
  köprüden geçemez.
- **İnternete açık bölge** (`septum-gateway`, `septum-audit`) —
  maskelenmiş LLM isteklerini Anthropic / OpenAI / OpenRouter'a iletir
  ve PII'siz uyumluluk telemetrisini loglar. Kod-review invariant'ı
  olarak bu paketler asla `septum-core`'u import etmez; Dockerfile
  `packages/core/`'u gateway veya audit image'larına hiç COPY etmeyerek
  bunu image-katmanında zorlar.

| Bölge | Paket | Rol |
|:---|:---|:---|
| Hava boşluklu | `septum-core` | PII tespit, maskeleme, demaskeleme, regülasyon motoru. Sıfır ağ bağımlılığı. |
| Hava boşluklu | `septum-mcp` | Claude Code / Desktop / Cursor'a stdio üzerinden core araçlarını açan MCP sunucu. |
| Hava boşluklu | `septum-api` | FastAPI REST uç noktaları, doküman pipeline, kimlik doğrulama, hız sınırlama. |
| Hava boşluklu | `septum-web` | Next.js 16 panel (App Router + React 19). |
| Köprü | `septum-queue` | Soyut `QueueBackend` Protocol + envelope dataclass'ları; dosya / Redis Streams somut backend'ler. |
| İnternete açık | `septum-gateway` | Bulut LLM yönlendiricisi. Kuyruktan maskelenmiş istekleri tüketir ve maskelenmiş cevapları geri yayınlar. |
| İnternete açık | `septum-audit` | Append-only JSONL sink + JSON / CSV / Splunk HEC dışa aktarıcıları. İsteğe bağlı kuyruk tüketici. |
...
```
packages/
├── core/                 # septum-core (hava boşluklu; sıfır ağ deps)
│   ├── septum_core/
...
│
├── mcp/                  # septum-mcp (hava boşluklu; stdio MCP sunucu)
│   ├── septum_mcp/server.py, tools.py, config.py
│   └── pyproject.toml          # ek bileşenler: [test]
│
├── api/                  # septum-api (hava boşluklu; FastAPI)
│   ├── septum_api/
...
│
├── web/                  # septum-web (hava boşluklu; Next.js 16 panel)
...
│   │   ├── file_backend.py       # POSIX atomik rename, hava boşluğu varsayılanı
│   │   └── redis_backend.py      # Redis Streams consumer groups
│   └── pyproject.toml          # ek bileşenler: [redis], [test]
│
├── gateway/              # septum-gateway (internete açık)
│   ├── septum_gateway/
...
│
└── audit/                # septum-audit (internete açık)
    ├── septum_audit/
...
| Hava boşluklu bölge | `docker-compose.airgap.yml` | api + web + Postgres + Redis (gateway yok). `USE_GATEWAY_DEFAULT=true` bulut çağrılarını Redis Streams üzerinden yönlendirir. | İki-host ayrık kurulumunun iç host'u. |
| İnternete açık bölge | `docker-compose.gateway.yml` | gateway-worker + gateway-health + audit-worker + audit-api + Redis. YAML anchor'lar (`x-gateway-base`, `x-audit-base`) ile servis tanımları tekrarı kaldırılmıştır. | İki-host ayrık kurulumunun DMZ / bulut host'u. |

Gerçek bir hava boşluklu kurulum için `airgap.yml`'ı iç host'ta ve
...
İnternete açık bölge için bulut LLM yönlendiricisi:
