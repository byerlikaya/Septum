...
Septum, `packages/` altında yer alan **yedi bağımsız kurulabilir pakete**
bölünmüştür; bu paketler üç ayrı güvenlik bölgesine dağıtılır:
...
| Bölge | Paket | Rol |
|:---|:---|:---|
| Air-gapped | `septum-core` | PII tespit, maskeleme, geri yazma, regülasyon motoru. Ağ bağımlılığı yok. |
...
Genel hat şu adımlardan oluşur:
...
3. **Katman 2 — Dile özgü NER**
   ...
   - Bu katman:
     ...
     - Cihazı otomatik seçer (CUDA → MPS → CPU) ve cache'lenmiş pipeline'lar sayesinde hızlı çalışır.
...
5. **Anonimleştirme ve coreference**
   - Yukarıdaki katmanlardan çıkan tüm span'ler birleştirilir, yinelenenler ayıklanır ve `AnonymizationMap` içine aktarılır:
     ...
     - **Blocklist** yalnızca kişi-tanımlayıcı varlık tipleriyle (PERSON_NAME, FIRST_NAME, LAST_NAME, ALIAS, USERNAME) sınırlandırılmıştır; böylece yaygın kelimeler yanlışlıkla maskelenmez.
...
4. Dönen cevaptaki placeholder'lar, bellekte tutulan anonimleştirme haritası üzerinden yerelde gerçek değerlere çevrilir.
...
Sohbet isteğinde `document_ids` verilmediğinde (ya da boş bırakıldığında) Septum, doküman araması mı yapacağına yoksa doğrudan sohbet yoluyla mı yanıt vereceğine kendisi karar verir. Yerel Ollama niyet sınıflandırıcısı sorguyu inceleyerek `SEARCH` veya `CHAT` döndürür. Üç yol oluşur:
...
3. **Düz LLM** — seçim yok, sınıflandırıcı `CHAT` diyor ya da hiçbir parça eşiği aşmıyor. LLM'e doküman bağlamı eklenmez.
...
test paketiyle birlikte gelir. Her paket bağımsız olarak kurulabilir ve
test edilebilir (`pip install -e "packages/<ad>[<extra'lar>]"`).
...
FastAPI tarafında Context7 en iyi uygulamaları takip edilir:
...
| Full dev stack | `docker-compose.yml` | api + web + gateway-worker + audit-worker + audit-api + Postgres + Redis + isteğe bağlı Ollama profili | Yerel geliştirme ya da zone ayrımını koruyarak tek host'ta kurulum. |
...
hafiftir (~250 MB, torch / Presidio / spaCy yok); image katmanında
uygulanan kural gereği runtime stage'e `packages/core/` kopyalanmaz.
...
PII motoru: tespit, maskeleme, geri yazma, regülasyon kompozisyonu.
...
`dev.sh --setup` her `packages/*` modülünü geliştirme extra'larıyla
editable modda yükler (`septum-core[transformers,test]`,
...
- **Modüler paket testleri** `packages/<ad>/tests/` altında — izole ve
  hızlıdır; her paketin testleri repo'nun geri kalanı olmadan
  çalışabilir (`pytest packages/<ad>/tests/`).
- **API entegrasyon testleri** `packages/api/tests/` altında — tam
  doküman ve sohbet pipeline'ını uçtan uca işletir. Bootstrap,
  database, router'lar, servisler, utils ve auth middleware burada test
  edilir.
