...
export function DocumentPreview({
  ...
  useEffect(() => {
    ...
    const fetchChunksAndSchema = async (): Promise<void> => {
      ...
      const isTabular =
        document.file_format === "xlsx" ||
        document.file_format === "xls" ||
        document.file_format === "ods" ||
        document.file_format === "csv" ||
        document.file_format === "tsv";
  ...
  const rawContentHeading = t(
    "preview.document.contentHeading",
    "Sanitized content"
  );
  const contentHeading =
    rawContentHeading === "preview.document.contentHeading"
      ? "Sanitized content"
      : rawContentHeading;
                    ...
                    {schema.columns.length === 0 ? (
                      ...
                      <>
                        <p className="text-[11px] text-slate-400">
                          Map generic column labels to semantic roles. Avoid entering
                          any raw personal data here.
                        </p>
                          ...
                          <table className="w-full border-collapse text-[11px]">
                            <thead>
                              <tr className="border-b border-slate-800 text-slate-400">
                                ...
                                <th className="px-1 py-1 text-left">Numeric</th>
                              </tr>
                            </thead>
                            <tbody>
                                      ...
                                      <input
                                        ...
                                        onChange={event =>
                                          handleSchemaFieldChange(
                                            ...
                                            current => ({
                                              ...current,
                                              is_numeric: event.target.checked
                                            })
                                          )
                                        }
                                      />
